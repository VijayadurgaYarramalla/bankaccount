...
def managingRequests():
    ...
    inp=input("enetr req_type to fetch pending,   rejected, approved") 
...
def deleteCustomer():
    deletPersonuser_id=int(input("enter user_id here to dlete that customer :---   "))
...
def withdraw(userid):
    amt=int(input("enetr amount to drwa :----     "))
...
def login():
    ...
    if userR == "customer":
        ...
        choose=input("enetr one option here :----      ") # "1" 2 3
        ...
        if choose == "4":
            ...
            req_Type=int(input("enetr yr choice here :--     "))
            if req_Type == 1:
                req="loan"
